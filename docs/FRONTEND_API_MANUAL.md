...
**Workflow / UI integration note:** Only a current `draft` can be changed. A draft may transition once to `archived`; generated, approved, and archived reports return `409 CONFLICT`. `mission_id`, `site_id`, `generated_by`, and `approved_by` are server-owned. Update plus `report.update` audit evidence are atomic. After success, replace cached detail and invalidate RPT-01; after archiving, disable edit/generate controls.
