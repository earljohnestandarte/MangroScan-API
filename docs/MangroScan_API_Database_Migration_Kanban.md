...
| DCL boundary | `045_validation_foundation_grants.sql` keeps the base tables closed by default. VAL-01, VAL-02 and VAL-04 reuse versioned SELECT privileges. `048_validation_session_creation_grants.sql` adds only the columns required to INSERT an open session; `050_ground_truth_creation_grants.sql` adds approved GT-01 INSERT columns; `051_validation_decision_grants.sql` adds approved MATCH-01 INSERT columns plus only the canonical tree columns its workflow may update. Worker/report/auditor mutation remains denied. |
...
| Tests / status | The VAL-01..04, GT-01, and MATCH-01 suites verify exact safe projections, tenant and lineage constraints, GeoJSON input/output, decision-specific references, strict domains, server-derived errors, canonical tree transitions, false-decision accuracy handoff, authentication/RBAC/inactivity/throttling, rollback-safe audit, duplicate conflicts, routes, and least-privilege DCL. MATCH-focused SQLite passes 38 tests / 382 assertions with three expected PostgreSQL-only skips; focused PostgreSQL/PostGIS passes 38 / 387. Full SQLite passes 729 / 4914 with twelve expected PostgreSQL-only skips; full PostgreSQL passes 729 / 4949. Migration rollback/reapply, live four-role DCL, Pint, routes, schema, Composer, and tracker-integrity gates pass. VAL-01 through MATCH-01 are Done; ACC-01 and VAL-05 handoff verification is next. |
