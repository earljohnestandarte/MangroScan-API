...
| DCL boundary | `045_validation_foundation_grants.sql` keeps the base tables closed by default. VAL-01, VAL-02 and VAL-04 reuse versioned SELECT privileges. `048_validation_session_creation_grants.sql` adds only open-session INSERT columns; `050_ground_truth_creation_grants.sql` adds GT-01 INSERT columns; `051_validation_decision_grants.sql` adds MATCH-01 INSERT and canonical tree-update columns. `052_accuracy_recompute_grants.sql` corrects the earlier generic metric grant by revoking table-level insert/update/delete, granting only recompute INSERT/UPDATE columns, and leaving DELETE denied. Worker/report/auditor mutation remains denied. |
...
| Tests / status | The VAL-01..04, GT-01, MATCH-01, and ACC-01 suites verify exact safe projections, tenant and lineage constraints, GeoJSON input/output, decision-specific references, strict domains, server-derived errors/metrics, stable recompute identities, canonical tree transitions, authentication/RBAC/inactivity/throttling, rollback-safe audit, duplicate/protocol conflicts, routes, and least-privilege DCL. ACC-01-focused SQLite and PostgreSQL/PostGIS each pass 10 tests / 84 assertions. Full SQLite passes 739 / 4998 with twelve expected PostgreSQL-only skips; full PostgreSQL passes 739 / 5033. Migration rollback/reapply, live four-role DCL, Pint, routes, schema, Composer, and tracker-integrity gates pass. VAL-01 through ACC-01 are Done; VAL-05 handoff verification is next. |
