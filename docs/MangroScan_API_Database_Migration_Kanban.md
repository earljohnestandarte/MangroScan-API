...
| Implementation field | Detail |
| :---- | :---- |
| Scope | Reproduces the authoritative base `validation_sessions`, `ground_truth_tree_records`, `validation_matches` and `accuracy_metrics` tables. UUID models and inverse relationships connect missions, sites, plots, users, species, tree observations and model versions. VAL-01 now provides the read-only scope/options implementation; it remains in Testing until PostgreSQL verification succeeds. Later mutation endpoints remain independently gated. |
...
| DCL boundary | `045_validation_foundation_grants.sql` keeps the base tables closed by default. VAL-01 reuses the minimum SELECT privileges already versioned for identity, sites, missions, plots, species and validation sessions; it adds no mutation privilege. Later INSERT/UPDATE privileges remain unavailable until their endpoint contracts are implemented. |
...
| Tests / status | `ValidationScopeTest` verifies the exact safe projection, stable ordering, tenant and mission/site lineage, active species, eligible assignees, authentication, local/foreign RBAC, inactivity, throttling, no audit side effect, route registration and all reused SELECT grants. Focused validation SQLite passes 12 tests / 157 assertions with one expected PostgreSQL-only skip; full SQLite passes 666 / 4419 with ten skips. The focused PostgreSQL profile is safely pinned to `mangroscan_test` but errors before migrations because `.env.testing` resolves `mangroscan_dev`, which is denied `CREATE` on schema `public`. Keep VAL-01 in Testing until the dedicated `mangroscan_test` owner is configured and the PostgreSQL gates pass. |
...
| VAL-01 | GET /validation/scopesMission/site/plot/species/assignee options. | No body | 200 {data:{missions,species,assignees,sessions}} | MSN/SITE/USR | **P0** | Earljohn Estandarte | **Testing** |
| VAL-02 | GET /validation-sessionsList field validation sessions. | Query: mission\_id?,site\_id?,status?,page | 200 {data:\[ValidationSession\],meta} | VAL-01 | **P0** | Earljohn Estandarte | **Blocked** |
| VAL-03 | POST /validation-sessionsCreate mission-scoped validation activity. | {mission\_id,site\_id,plot\_id?,validated\_by,validation\_date,method,notes?} | 201 {data:ValidationSession} | VAL-01 \+ TREE-01 | **P0** | Earljohn Estandarte | **Blocked** |
| VAL-04 | GET /validation-sessions/{id}Validation workspace data and map layers. | Path: id | 200 {data:{context,observations,ground\_truth\_records,matches,metrics,layers}} | VAL-03 | **P0** | Earljohn Estandarte | **Blocked** |
| GT-01 | POST /validation-sessions/{id}/ground-truthCreate manual field tree record. | {field\_code?,species\_id?,location:GeoJSON,height\_m?,age\_years?,diameter\_cm?,crown\_diameter\_m?,health\_status,is\_tree,photo\_path?,notes?} | 201 {data:GroundTruthRecord} | VAL-04 | **P0** | Earljohn Estandarte | **Blocked** |
