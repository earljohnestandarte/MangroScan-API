...
| Implementation field | Detail |
| :---- | :---- |
| Scope | Reproduces the authoritative base `validation_sessions`, `ground_truth_tree_records`, `validation_matches` and `accuracy_metrics` tables. UUID models and inverse relationships connect missions, sites, plots, users, species, tree observations and model versions. VAL-01 provides scope/options, VAL-02 provides the filtered session index, and VAL-03 creates audited validation activities. Later mutation endpoints remain independently gated. |
...
| DCL boundary | `045_validation_foundation_grants.sql` keeps the base tables closed by default. VAL-01 and VAL-02 reuse versioned SELECT privileges. `048_validation_session_creation_grants.sql` adds only the columns required to INSERT an open session for `mangroscan_api_rw`; completion columns and worker/auditor INSERT remain denied. |
...
| Tests / status | The VAL-01..03 suites verify exact safe projections, stable ordering/pagination, composed filters, tenant and mission/site/plot lineage, active option/assignee eligibility, authentication, local/foreign RBAC, inactivity, throttling, no read-side audit, mandatory rollback-safe creation audit, duplicate conflict behavior, routes, and least-privilege DCL. Focused validation passes 25 tests / 222 assertions on both SQLite and PostgreSQL/PostGIS. Full SQLite passes 685 / 4572 with ten expected PostgreSQL-only skips; full PostgreSQL 18/PostGIS passes 695 / 4605. Live DCL, Pint, route, schema, and trigger gates pass. VAL-01 through VAL-03 are Done; VAL-04 is dependency-ready. |
...
| VAL-04 | GET /validation-sessions/{id}Validation workspace data and map layers. | Path: id | 200 {data:{context,observations,ground\_truth\_records,matches,metrics,layers}} | VAL-03 | **P0** | Earljohn Estandarte | **Not Done — Ready** |
