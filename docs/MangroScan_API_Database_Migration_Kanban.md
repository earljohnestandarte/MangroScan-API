...
| Implementation field | Detail |
| :---- | :---- |
| Scope | Reproduces the authoritative base `validation_sessions`, `ground_truth_tree_records`, `validation_matches` and `accuracy_metrics` tables. UUID models and inverse relationships connect missions, sites, plots, users, species, tree observations and model versions. VAL-01 provides completed scope/options and VAL-02 provides the completed filtered session index. Later mutation endpoints remain independently gated. |
...
| DCL boundary | `045_validation_foundation_grants.sql` keeps the base tables closed by default. VAL-01 and VAL-02 reuse the versioned SELECT privileges for their tenant-safe reads and add no mutation privilege. Any workflow mutation privileges remain independently governed and are not exposed through these GET routes. |
...
| Tests / status | `ValidationScopeTest` and `ValidationSessionIndexTest` verify exact safe projections, stable ordering/pagination, composed filters, tenant and mission/site/plot lineage, active option eligibility, authentication, local/foreign RBAC, inactivity, throttling, no read-side audit, route registration and reused SELECT grants. Focused validation passes 14 tests / 129 assertions on both SQLite and PostgreSQL/PostGIS. Full SQLite passes 674 / 4479 with ten expected PostgreSQL-only skips; full PostgreSQL 18/PostGIS passes 684 / 4512. Pint, route, schema/trigger, and DCL gates pass. VAL-01 and VAL-02 are Done. |
