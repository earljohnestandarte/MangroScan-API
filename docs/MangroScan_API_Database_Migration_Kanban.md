...
| Implementation field | Detail |
| :---- | :---- |
| Scope | Reproduces the authoritative base `validation_sessions`, `ground_truth_tree_records`, `validation_matches` and `accuracy_metrics` tables. UUID models and inverse relationships connect missions, sites, plots, users, species, tree observations and model versions. VAL-01 provides the completed read-only scope/options implementation. Later mutation endpoints remain independently gated. |
...
| Tests / status | `ValidationScopeTest` verifies the exact safe projection, stable ordering, tenant and mission/site lineage, active species, eligible assignees, authentication, local/foreign RBAC, inactivity, throttling, no audit side effect, route registration and all reused SELECT grants. Focused validation SQLite passes 12 tests / 157 assertions with one expected PostgreSQL-only skip; focused PostgreSQL VAL-01/foundation passes 10 / 129. Full SQLite passes 666 / 4425 with ten expected PostgreSQL-only skips; full PostgreSQL 18/PostGIS passes 676 / 4458. Route, Pint, Composer, schema/trigger, and DCL gates pass. VAL-01 is Done. |
