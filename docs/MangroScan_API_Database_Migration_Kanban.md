...
| Implementation field | Detail |
| :---- | :---- |
| Scope | Reproduces the authoritative base `validation_sessions`, `ground_truth_tree_records`, `validation_matches` and `accuracy_metrics` tables. UUID models and inverse relationships connect missions, sites, plots, users, species, tree observations and model versions. VAL-01 provides scope/options, VAL-02 provides the filtered session index, VAL-03 creates audited activities, and VAL-04 assembles the complete read-only workspace. Later mutation endpoints remain independently gated. |
...
| DCL boundary | `045_validation_foundation_grants.sql` keeps the base tables closed by default. VAL-01, VAL-02 and VAL-04 reuse versioned SELECT privileges across validation, tree-observation and geospatial-layer tables. `048_validation_session_creation_grants.sql` adds only the columns required to INSERT an open session for `mangroscan_api_rw`; status/completion columns and worker/auditor INSERT remain denied. |
| Unresolved contract conflicts | VAL-01 is resolved by nesting each site and its plots beneath the corresponding mission while preserving the approved `{missions,species,assignees,sessions}` envelope. Later cards still require fields absent from the authoritative schema: GT-01 requests `field_code`, `crown_diameter_m`, `is_tree` and `notes` while the table instead has `remarks`; MATCH-01 requests accepted/corrected/evidence fields not present in `validation_matches`. Those decisions remain independently gated. The nullable tree-observation FK follows the explicit reference DDL and false-negative workflow. |
| Tests / status | The VAL-01..04 suites verify exact safe projections/envelopes, stable ordering/pagination, composed filters, tenant and mission/site/plot/validator lineage, active option/assignee eligibility, GeoJSON output, private-path exclusion, authentication, local/foreign RBAC, inactivity, throttling, no read-side audit, mandatory rollback-safe creation audit, duplicate conflicts, routes, and least-privilege DCL. Focused validation passes 32 tests / 295 assertions on both SQLite and PostgreSQL/PostGIS. Full SQLite passes 692 / 4645 with ten expected PostgreSQL-only skips; full PostgreSQL 18/PostGIS passes 702 / 4678. Six live workspace SELECT checks plus Pint, route, schema, and trigger gates pass. VAL-01 through VAL-04 are Done. GT-01 is Blocked pending an approved home for `field_code`, `crown_diameter_m`, and `is_tree`. |
...
| GT-01 | POST /validation-sessions/{id}/ground-truthCreate manual field tree record. | {field\_code?,species\_id?,location:GeoJSON,height\_m?,age\_years?,diameter\_cm?,crown\_diameter\_m?,health\_status,is\_tree,photo\_path?,notes?} | 201 {data:GroundTruthRecord} | VAL-04 | **P0** | Earljohn Estandarte | **Blocked — schema decision required** |
| MATCH-01 | POST /validation-sessions/{id}/decisionsStore matched/corrected/false-positive/false-negative decision. | {tree\_observation\_id?,ground\_truth\_id?,decision,accepted\_species\_id?,accepted\_height\_m?,accepted\_age\_years?,corrected\_geometry?,notes?,validation\_evidence?} | 201 {data:ValidationMatch} | VAL-04 \+ GT-01 | **P0** | Earljohn Estandarte | **Blocked** |
| ACC-01 | POST /validation-sessions/{id}/accuracy/recomputeRecompute precision/recall/F1/RMSE/MAE evidence. | No body | 200 {data:\[AccuracyMetric\]} | MATCH-01 | **P0** | Jason Benabente | **Blocked** |
| VAL-05 | POST /validation-sessions/{id}/completeComplete validation task. | {notes} | 200 {data:ValidationSession} | MATCH-01; protocol gate | **P1** | Jason Benabente | **Blocked** |
...
| DASH-01 | GET /dashboard/overviewRole-scoped KPI overview. | Query: site\_id?,mission\_id?,date range? | 200 {data:{missions,trees,species,validation,processing}} | TREE \+ ACC \+ materialized views | **P1** | Earljohn Estandarte | **Blocked** |
| DASH-02 | GET /dashboard/missions/{id}Mission analytics/detail dashboard. | Path: id | 200 {data:{counts,species,height,age,accuracy,layers}} | DASH-01 | **P1** | Earljohn Estandarte | **Blocked** |
