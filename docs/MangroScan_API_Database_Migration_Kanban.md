...
| DCL boundary | `045_validation_foundation_grants.sql` keeps the base tables closed by default. VAL-01, VAL-02 and VAL-04 reuse versioned SELECT privileges across validation, tree-observation and geospatial-layer tables. `048_validation_session_creation_grants.sql` adds only the columns required to INSERT an open session for `mangroscan_api_rw`; status/completion columns and worker/auditor INSERT remain denied. `050_ground_truth_creation_grants.sql` adds only the approved GT-01 INSERT columns to the API role. |
| Unresolved contract conflicts | VAL-01 is resolved by nesting each site and its plots beneath the corresponding mission while preserving the approved `{missions,species,assignees,sessions}` envelope. GT-01 is resolved by additive nullable `field_code`/`crown_diameter_m` and required default-true `is_tree` columns while mapping `notes` to canonical `remarks`. MATCH-01 remains the only validation-schema implementation still pending: its approved direct session lineage and accepted/corrected/evidence fields must be versioned before the decision endpoint is exposed. |
| Tests / status | The VAL-01..04 and GT-01 suites verify exact safe projections/envelopes, stable ordering/pagination, composed filters, tenant and mission/site/plot/validator lineage, active option/assignee eligibility, GeoJSON input/output, strict measurement domains, private-path exclusion, authentication, local/foreign RBAC, inactivity, throttling, no read-side audit, mandatory rollback-safe creation audit, duplicate conflicts, routes, and least-privilege DCL. GT-01-focused SQLite passes 23 tests / 224 assertions with two expected PostgreSQL-only skips; focused PostgreSQL/PostGIS passes 23 / 228. Full SQLite passes 717 / 4787 with eleven expected PostgreSQL-only skips; full PostgreSQL passes 717 / 4821. Migration rollback/reapply, live four-role DCL, Pint, route, schema, and tracker-integrity gates pass. VAL-01 through GT-01 are Done; MATCH-01 is next. |
...
| MATCH-01 | POST /validation-sessions/{id}/decisionsStore matched/corrected/false-positive/false-negative decision. | {tree\_observation\_id?,ground\_truth\_id?,decision,accepted\_species\_id?,accepted\_height\_m?,accepted\_age\_years?,corrected\_geometry?,notes?,validation\_evidence?} | 201 {data:ValidationMatch} | VAL-04 \+ GT-01 | **P0** | Earljohn Estandarte | **Not Done — contract approved; next** |
